...
def train_val(config, model1, model2, train_loader, val_loader, criterion):
    ...
    for epoch in range(epochs):
        ...
        for idx, (batch, batch_w_s) in enumerate(source_dataset):
            ...
            loss_1 = sup_loss_1 + unsup_loss_1 * consistency_weight * (
                sup_batch_len / unsup_batch_len
            )

            sup_loss_2 = (losses_l2[0] + losses_l2[1]) / 2.0
            unsup_loss_2 = (losses_u2[0] + losses_u2[1]) / 2.0
            loss_2 = sup_loss_2 + unsup_loss_2 * consistency_weight * (
                sup_batch_len / unsup_batch_len
            )

            # Loss dis - CCVC
            cos_dis = nn.CosineSimilarity(dim=2, eps=1e-6)
            lb_dis_loss = 1 + cos_dis(lb_features_1, lb_features_2).mean()
            ulb_dis_loss = 1 + cos_dis(ulb_features_1, ulb_features_2).mean()
            ...
            file_log.write(
                "Dice Sup Loss 1: {}, Dice Unsup Loss 1: {}, BCE Sup Loss 1: {}, BCE UnSup Loss 1: {}\n".format(
                    round(losses_l1[1].item(), 5),
                    round(losses_u1[1].item(), 5),
                    round(losses_l1[0].item(), 5),
                    round(losses_u1[0].item(), 5),
                )
            )
            file_log.write(
                "Dice Sup Loss 2: {}, Dice Unsup Loss 2: {}, BCE Sup Loss 2: {}, BCE UnSup Loss 2: {}\n".format(
                    round(losses_l2[1].item(), 5),
                    round(losses_u2[1].item(), 5),
                    round(losses_l2[0].item(), 5),
                    round(losses_u2[0].item(), 5),
                )
            )
            file_log.flush()
            ...
            print(
                "Dice Sup Loss 1: {}, Dice Unsup Loss 1: {}, BCE Sup Loss 1: {}, BCE UnSup Loss 1: {}".format(
                    round(losses_l1[1].item(), 5),
                    round(losses_u1[1].item(), 5),
                    round(losses_l1[0].item(), 5),
                    round(losses_u1[0].item(), 5),
            ...
            print(
                "Dice Sup Loss 2: {}, Dice Unsup Loss 2: {}, BCE Sup Loss 2: {}, BCE UnSup Loss 2: {}".format(
                    round(losses_l2[1].item(), 5),
                    round(losses_u2[1].item(), 5),
                    round(losses_l2[0].item(), 5),
                    round(losses_u2[0].item(), 5),
        ...
        file_log.flush()
        print(
            "Epoch {}, Total train step {} || AVG_loss: {}, Avg Dice score 1: {}, Avg IOU 1: {}, Avg Dice score 2: {}, Avg IOU 2: {}".format(
                epoch,
                iter,
                round(loss_train_sum / num_train, 5),
                round(dice_train_sum_1 / num_train, 4),
        ...
        loss_val_epoch, dice_val_epoch, iou_val_epoch = (
            loss_val_sum / num_val,
            dice_val_sum / num_val,
            iou_val_sum / num_val,
        )

        # print
        file_log.write(
            "Epoch {}, Model 1, Validation || sum_loss: {}, Dice score: {}, IOU: {}\n".format(
                epoch,
                round(loss_val_epoch.item(), 5),
                round(dice_val_epoch, 4),
                round(iou_val_epoch, 4),
            )
        )
        file_log.flush()
        print(
            "Epoch {}, Model 1, Validation || sum_loss: {}, Dice score: {}, IOU: {}".format(
                epoch,
                round(loss_val_epoch.item(), 5),
                round(dice_val_epoch, 4),
                round(iou_val_epoch, 4),
        ...
        # print
        file_log.write(
            "Epoch {}, Model 2, Validation || sum_loss: {}, Dice score: {}, IOU: {}\n".format(
                epoch,
                round(loss_val_epoch.item(), 5),
                round(dice_val_epoch, 4),
                round(iou_val_epoch, 4),
        ...
        file_log.flush()
        print(
            "Epoch {}, Model 2, Validation || sum_loss: {}, Dice score: {}, IOU: {}".format(
                epoch,
                round(loss_val_epoch.item(), 5),
                round(dice_val_epoch, 4),
                round(iou_val_epoch, 4),
        ...
        file_log.flush()
        print(
            "Training and evaluating on epoch{} complete in {:.0f}m {:.0f}s".format(
                epoch, time_elapsed // 60, time_elapsed % 60
            )
        )
...
def test(config, model, model_dir, test_loader, criterion):
    ...
    loss_test_epoch, dice_test_epoch, iou_test_epoch = (
        loss_test_sum / num_test,
        dice_test_sum / num_test,
        iou_test_sum / num_test,
    )

    # logging average and store results
    with open(test_results_dir, "w") as f:
        f.write(
            f"loss: {loss_test_epoch.item()}, Dice_score {dice_test_epoch}, IOU: {iou_test_epoch}"
        )
    ...
    file_log.write(
        "Test || Average loss: {}, Dice score: {}, IOU: {}\n".format(
            round(loss_test_epoch.item(), 5),
            round(dice_test_epoch, 4),
            round(iou_test_epoch, 4),
        )
    )
    file_log.flush()
    print(
    ...
    print(
        "Test || Average loss: {}, Dice score: {}, IOU: {}".format(
            round(loss_test_epoch.item(), 5),
            round(dice_test_epoch, 4),
            round(iou_test_epoch, 4),
        )
...
if __name__ == "__main__":
    ...
    exp_dir = "{}/{}_{}/fold{}".format(
        config.data.save_folder, args.exp, config["data"]["supervised_ratio"], args.fold
    )
