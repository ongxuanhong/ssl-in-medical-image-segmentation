...
class ResNet(nn.Module):
    ...
    def __init__(
        ...
        if 2 in out_indices:
            self.layer2 = self._make_layer(
                block, 128, layers[1], stride=2, adapt_method=adapt_method
            )
        else:
            self.layer2 = nn.Identity()
        if 3 in out_indices:
            self.layer3 = self._make_layer(
                block, 256, layers[2], stride=2, adapt_method=adapt_method
            )
        else:
        ...
        if 4 in out_indices:
            self.layer4 = self._make_layer(
                block, 512, layers[3], stride=2, adapt_method=adapt_method
            )
        if -1 in out_indices:
        ...
        self.upsample = nn.Sequential(
            nn.ConvTranspose2d(512 * block.expansion, 256, kernel_size=2, stride=2),
            ...
            nn.Conv2d(
                16, 1, kernel_size=1
            ),  # Final layer to get the desired output channels
        )
    ...
    def _make_layer(self, block, planes, blocks, stride=1, adapt_method=None):
        ...
        layers = []
        layers.append(
            block(self.inplanes, planes, stride, downsample, adapt_method=adapt_method)
        )
