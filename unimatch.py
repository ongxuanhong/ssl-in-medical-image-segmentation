...
def train_val(config, model, train_loader, val_loader, criterion):
    ...
    for epoch in range(epochs):
        ...
        for idx, (
            ...
            mask_u_w_cutmixed1[cutmix_box1.unsqueeze(1) == 1] = mask_u_w_mix[
                cutmix_box1.unsqueeze(1) == 1
            ]
            conf_u_w_cutmixed1[cutmix_box1.unsqueeze(1) == 1] = conf_u_w_mix[
                cutmix_box1.unsqueeze(1) == 1
            ]

            mask_u_w_cutmixed2[cutmix_box2.unsqueeze(1) == 1] = mask_u_w_mix[
                cutmix_box2.unsqueeze(1) == 1
            ]
            conf_u_w_cutmixed2[cutmix_box2.unsqueeze(1) == 1] = conf_u_w_mix[
                cutmix_box2.unsqueeze(1) == 1
            ]
            ...
            loss = (
                sup_loss
                + unsup_loss * (sup_batch_len / unsup_batch_len) * consistency_weight
            )
            ...
            file_log.write(
                "Epoch {}, iter {}, Sup Loss: {}, Unsup Loss: {}\n".format(
                    epoch,
                    iter + 1,
            ...
            file_log.flush()
            print(
                "Epoch {}, iter {}, Sup Loss: {}, Unsup Loss: {}".format(
                    epoch,
                    iter + 1,
                    round(sup_loss.item(), 5),
                    round(unsup_loss.item(), 5),
        ...
        # print
        file_log.write(
            "Epoch {}, Validation || sum_loss: {}, Dice score: {}, IOU: {}\n".format(
                epoch,
        ...
        print(
            "Epoch {}, Validation || sum_loss: {}, Dice score: {}, IOU: {}".format(
                epoch,
                round(loss_val_epoch.item(), 5),
                round(dice_val_epoch, 4),
                round(iou_val_epoch, 4),
        ...
        wandb.log(
            {
                ...
                "Test/Dice_score": dice_test_epoch,
